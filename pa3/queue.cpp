...
template <class T> void Queue<T>::enqueue(T const &newItem)
{
    ...
    //add 1 element to the end of stack 1 O(1)
...
template <class T> T Queue<T>::dequeue()
{
    ...
    // if stack 2 is empty, push things from stack 1 to stack 2
    // this will take O(n) times but this case only happens once per n dequeue
    // so cost is O(n)/n = O(1)
    if(stack_2.size() == 0){
    ...
    // pop 1 item from stack 2 take O(1)
    T ret = stack_2.pop();
...
template <class T> T Queue<T>::peek()
{
    ...
    // peek 1 item from stack 2 take O(1)
    T ret = stack_2.peek();
